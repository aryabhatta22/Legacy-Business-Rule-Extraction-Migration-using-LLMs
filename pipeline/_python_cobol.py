...
def process_cobol(lines):
    return clean_names(
        denormalize_cobol(parse_cobol(clean_cobol(lines))), True, True, True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip-all-processing",
        help="Only processes the redefines.",
        default=False,
        action="store_true",
    )
    parser.add_argument(
        "--skip-unique-names",
        help="Skips making all names unique.",
        default=False,
        action="store_true",
    )
    parser.add_argument(
        "--skip-denormalize",
        help="Skips denormalizing the COBOL.",
        default=False,
        action="store_true",
    )
    parser.add_argument(
        "--skip-strip-prefix",
        help="Skips stripping the prefix from the names.",
        default=False,
        action="store_true",
    )
    ...
    with open(args.filename, "r") as f:
        ...
        if not args.skip_all_processing:
            ...
            if not args.skip_strip_prefix or not args.skip_unique_names:
                lines = clean_names(
                    lines, not args.skip_unique_names, not args.skip_strip_prefix
                )
